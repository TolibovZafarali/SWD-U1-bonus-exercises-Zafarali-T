...
function printItems(indexNum) {
	console.log("Items at index: " + indexNum)
	for (let i in pantry) {
		if (indexNum < pantry[i].length) {
			console.log(`\t${pantry[i][indexNum]}`)
		}
	}
}
...
function printItemsFromPackage(packageType) {
	try {
		console.log(`Items that are ${packageType}: `)
		for (let item of pantry[packageType]) {		
			console.log(`- ${item}`)
		}
	} catch {
		console.error("Package type is not found!")
	}
}
...
printItemsFromPackage("bagged")
printItemsFromPackage("boxed")
printItemsFromPackage("asdfj")
