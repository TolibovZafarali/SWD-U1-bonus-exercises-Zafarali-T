...
let zoo = {
	...
	adultTicketPrice: 50,
	childTicketPrice: 0,
	famousFeatures: ["Aquariums", "Trains", "Educational programs"]
}
...
let garden = {
	...
	adultTicketPrice: 16,
	childTicketPrice: 16,
	famousFeatures: ["Beautiful horticultural displays", "Extensive botanical research", "Dedication to plant conservation"]
}
...
for (let index of attractions) {
	for (let j in index) {
		if (Array.isArray(index[j])) {
			for (let arrVal of index[j]) {
				console.log(`Famous for: ${arrVal}`)
			}
		} else {
			console.log(`${j.toUpperCase()}: ${index[j]}`)
		}
	}
}
...
**********************************************
A favorite with the locals, the ${attraction.name} boasts over a dozen features, including the ${attraction.famousFeatures[0].toLowerCase()}, ${attraction.famousFeatures[1].toLowerCase()}, and ${attraction.famousFeatures[2].toLowerCase()}. Admission is $${attraction.adultTicketPrice} for adults, $${attraction.childTicketPrice} for children.`)
}